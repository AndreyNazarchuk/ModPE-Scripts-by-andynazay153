...
[mob1, mob2, mob3, etc.]
in which only mob1 and mob2 are required and each mob can either be the entity type ID of the mob you want or can be an array taking this structure:
...
Player.removeItemInventory(ID, amount, damage); //Takes away the specified amount of the item with the specified ID and damage from the player's inventory; if only the ID is entered, all items with that ID will be removed
Player.clearInventory(); //Clears the player's inventory
Player.checkForInventoryItem(ID, amount, damage); //Checks if there is an item with the specified ID, amount, and damage is in the inventory; only the ID is required
...
Item.defineItem(ID, unlocalizedName, localizedName, stackLimit, maxDamage, category, textureName, additionalTextures); //Creates a new item with all the specified statuses; only the ID is actually required
...
Block.defineBlock(ID, unlocalizedName, localizedName, textures, baseMaterial, isOpaque, renderShape, destroyTime, explosionResistance, lightLevel, color, renderLayer, shape); //Creates a new block with the specified statuses. The parameter color is an array, like in Block.setColor(), and the parameter shape is an array, containing the x1, y1, z1, x2, y2, and z2 parameters; although this method is already defined, more parameters have been added*/
