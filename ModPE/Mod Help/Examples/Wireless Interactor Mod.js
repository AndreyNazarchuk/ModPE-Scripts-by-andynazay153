//Wireless Interactor Mod
//by wilco375
//Don't re-upload this code, nor share or redistribute this mod using the direct download link. Instead, use this link: 
...
//Define Wireless Interactor & Wireless Connector
Block.defineBlock(Camera,"Wireless Interactor",["chest_inventory",2],5,false,0)
Block.setDestroyTime(Camera, 1)
Item.addCraftRecipe(Camera,1,0,[331,4,0,54,1,0,331,4,0])
Item.setCategory(Camera,8,0);

ModPE.setItem(Monitor,"item_frame",0,"Wireless Connector")
...
function selectLevelHook(){
 if(ModPE.readData("cameras"+Level.getWorldDir()) != null && ModPE.readData("cameras"+Level.getWorldDir()) != ""){
  ...
  print("Loaded "+cameras.length+" wireless interactors")
 }
}
...
function leaveGame(){
 if(camera == 1){
  ...
  setTile(cameraX,cameraY,cameraZ,Camera)
 ...
 print("Saved " + cameras.length + " wireless interactors")
 cameras = null
 camerasX = null
 camerasY = null
 camerasZ = null
...
function useItem(x,y,z,itemId,blockId,side){
 if(itemId == Monitor){
  ...
  if(cameras.length >= 32){
   clientMessage("Warning: You cannot see more than 32 wireless interactors in the GUI list.")}
  clientMessage("Please type »/wi name <wireless interactor name>« to give this camera a name")
  nameCam = 1
  if(side == 0){
   camX = x
   camY = y-1
...
function procCmd(command){
 ...
 //Give the camera a name
  if(cmd[1] == "name"){
   if(nameCam == 1){
    clientMessage("You named the wireless interactor " + cmd[2])
    if(cameras == null){
     cameras = []
     camerasX = []
     camerasY = []
   ...
   else{clientMessage("Place down a wireless interactor first to name it")}
  }
  //List all the cameras
  else if(cmd[1] == "interactors"){
  ...
  //Show a camera
   ...
   if(goToCam == null){
    clientMessage("This wireless interactor doesn't exist. Type »/wi interactors« to get a list of all the cameras")
   }
   else{
    if(camera != 1){
     ...
     if(getTile(cameraX,cameraY,cameraZ) == Camera){
      ...
      clientMessage("Type »/wi leave« to leave the camera")
      camera = 1
     }
     else{clientMessage("The block at the location of this wireless interactoe isn't a wireless interactor anymore!")}
    }
    else{
	 prevCameraX = cameraX
	 prevCameraY = cameraY
     ...
     else{clientMessage("The block at the location of this wireless interactor isn't a wireless interactor anymore!")}
    }
   }
   */
  }
  //Leave the current camera
   ...
   else{clientMessage("You're not looking through a wireless interactor")}
  }
  //Remove a camera from the list
   ...
   if(removed != 1) clientMessage("No wireless interactor with this name found")
   removed = 0
  }
  //List all the commands
  else if(cmd[1] == "list"){
   clientMessage("»/wi name <wireless interacroe name>«, used to set the name of a wireless interactor once prompted to do so.")
   clientMessage("»/wi interactors«, gives a list of all the wireless interactors")
   clientMessage("»/wi leave«, leaves the wireless interactor you're looking through")
   clientMessage("»/wi remove <wireless interactor name>«, a wireless interactor should automatically be removed from the list when broken. If not, you can use this command to remove it")
  }
 }
}
 ...
 ctx.runOnUiThread(new java.lang.Runnable(){
  run: function(){
   try{
    ...
    dialog.setTitle("Wireless Interactors");
    ...
    if(camera == 1){
     ...
     leave.setText("Leave this interactor")
...
function ShowCameraFromList(){
 if(camera != 1){
  ...
  if(getTile(cameraX,cameraY,cameraZ) == Camera){
   ...
   Entity.setPosition(getPlayerEnt(), cameraX,cameraY,cameraZ)
   clientMessage("Type »/wi leave« to leave the wireless interactor")
   camera = 1
  }
  else{
	clientMessage("The block at the location of this wireless interactor isn't a wireless interactor anymore!")
  ...
  if(getTile(cameraX,cameraY,cameraZ) == Camera){
   ...
   setTile(prevCameraX,prevCameraY,prevCameraZ,Camera)
   if(getTile(cameraX,cameraY-2,cameraZ) == 0){
    air = 1
    setTile(cameraX,cameraY-2,cameraZ,20)
   }
